...
const contactInfo = [
  {
    ...
    label: "Email",
    value: "info@greendeltaenvironment.com",
    href: "mailto:info@greendeltaenvironment.com",
  },
  {
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <ul className="space-y-5">
                {contactInfo.map((c) => (
                  <li key={c.label} className="flex items-start gap-4">
                    <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center rounded-xl bg-green-100 text-green-700">
                      {c.icon}
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 font-medium mb-0.5">{c.label}</p>
                      {c.href ? (
                        <a href={c.href} className="text-gray-800 hover:text-green-600 transition-colors text-sm font-medium break-all">
                          {c.value}
                        </a>
                      ) : (
